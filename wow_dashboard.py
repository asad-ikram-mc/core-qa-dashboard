...
<div class="main">
  ...
  <div class="sh"><span class="dot" style="background:var(--amber)"></span>Platform Data Coverage</div>
  <div class="sub-note" style="font-size:11px;color:var(--text-dim);margin:-10px 0 14px 15px">Rows = Platform Granular · Columns = Week Ending · Cells = post count (colored by impressions) · Red = missing data</div>
  <div class="tc"><div class="ts" id="coverage-tbl" style="max-height:600px"></div></div>

  <!-- Section: Country Data Coverage -->
  <div class="sh"><span class="dot" style="background:var(--amber)"></span>Country Data Coverage</div>
  <div class="sub-note" style="font-size:11px;color:var(--text-dim);margin:-10px 0 14px 15px">Rows = Country · Columns = Week Ending · Cells = post count (colored by impressions) · Red = missing data</div>
  <div class="tc"><div class="ts" id="coverage-co-tbl" style="max-height:600px"></div></div>
...
function go(){{
  ...
  renderKPIs();renderAggregate();renderPlatform();renderCountry();renderAnomalies();renderCoverage();renderRaw();
}}
...
function renderCoverage(){{
  const allWks=[...new Set(filtered.map(r=>r.week_ending))].sort();
  const platforms=[...new Set(filtered.map(r=>r['Platform Granular']))].sort();
  const countries=[...new Set(filtered.map(r=>r.Country))].sort();

  // Build lookup: dim -> week -> {{posts, imp}}
  function buildLookup(dimKey){{
    const m={{}};
    filtered.forEach(r=>{{
      const d=r[dimKey]||'Unknown', w=r.week_ending;
      if(!m[d])m[d]={{}};
      if(!m[d][w])m[d][w]={{posts:0,imp:0}};
      m[d][w].posts+=(r.post_count||0);
      m[d][w].imp+=(r.total_impressions||0);
    }});
    return m;
  }}

  function covClass(imp){{
    if(imp>=1000000) return 'cov-high';
    if(imp>=100000) return 'cov-mid';
    return 'cov-low';
  }}

  function renderTable(dims, lookup, targetId){{
    let h='<table><thead><tr><th class="cov-dim">Name</th>';
    allWks.forEach(w=>{{h+=`<th class="cov-head">${{w.slice(5)}}</th>`}});
    h+='</tr></thead><tbody>';

    dims.forEach(dim=>{{
      h+=`<tr><td class="cov-dim">${{dim}}</td>`;
      allWks.forEach(w=>{{
        const cell=lookup[dim]?.[w];
        if(!cell||cell.posts===0){{
          h+=`<td class="cov-miss">✗</td>`;
        }}else{{
          h+=`<td class="${{covClass(cell.imp)}}" title="${{fmt(cell.imp)}} impressions">${{cell.posts}}</td>`;
        }}
      }});
      h+='</tr>';
    }});
  ...
  renderTable(platforms, buildLookup('Platform Granular'), 'coverage-tbl');
  renderTable(countries, buildLookup('Country'), 'coverage-co-tbl');
}}
